...
from src.utils.general_util import printf, make_parent_directories, symlink
from src.utils.experiments_util import pandas2hdf
import inspect
...
def get_selected_submodel_name(model="spatiotemporal_mean",
    ...
    in src.models.{model}.attributes. If return_params is True, returns
    ...
    """
    try:
        attr = import_module(f"src.models.{model}.attributes")
    
        # Check if modules get_seleted_selected_submodel name supports return_params
        if "return_params" in inspect.getargspec(attr.get_selected_submodel_name).args:
            return attr.get_selected_submodel_name(gt_id, horizon, return_params)   
        else:
...
def get_submodel_name(model, *args, **kwargs):
    ...
    in src.models.{model}.attributes
